...
public class MainActivity extends AppCompatActivity {
   ...
   // private float Kp, Ki, Kd;
    ...
    public void newparams()
    {
        ...
        s+="%\n ";
        textView.setText(s);
        sendbuf[0] ='D';		// QCSVB - Let it be control_unit->drone protocol ID
        sendbuf[1] ='R';
        sendbuf[2] ='O';
        sendbuf[3] ='N';
        sendbuf[4] ='E';
...
//                }
        ...
        SeekBar.OnSeekBarChangeListener l2;
        l2=new SeekBar.OnSeekBarChangeListener()
        {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progresValue, boolean fromUser)
            {
                ...
                while (threadsalive)
                {
                    if (dataready)
                    {
                        try
                        {
                            ...
                            DatagramPacket packet = new DatagramPacket(sendbuf, BUFSIZE, InetAddress.getByAddress(bcastadr), 5880);
                            sockets.send(packet);
            ...
            public void run()
            {
                while (threadralive)
                {
                    try
                    {
                        ...
                        if ((receivebuf[0]=='D') && (receivebuf[1]=='R') && (receivebuf[2]=='O') && (receivebuf[3]=='N') && (receivebuf[4]=='E'))
                        {
